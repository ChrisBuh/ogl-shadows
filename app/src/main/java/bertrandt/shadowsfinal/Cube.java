...
public class Cube {
    ...
    public Cube(Context context, float[] center, float size, float[] color) {
        ...
        cubeTexel = ByteBuffer.allocateDirect(cubeTexelData.length * RenderConstants.FLOAT_SIZE_IN_BYTES)
                .order(ByteOrder.nativeOrder()).asFloatBuffer();
        cubeTexel.put(cubeTexelData).position(0);

        mCubeTextureHandle = TextureHelper.loadTexture(context, R.drawable.ab);
